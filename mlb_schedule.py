...
df = pd.DataFrame({'a': ['rk'],
    'b':['boxscore'],
                   ...
                   'e':['opp'],
                    'f':['r'],'g':['ra'],'h':['inn']})
df.to_csv("mlb_schedule_2014.csv",header=False,index=False)
def schedule(x):
    url='http://www.baseball-reference.com/teams/'+x+'/2014-schedule-scores.shtml'
    h = requests.get(url)
    ...
    df.to_csv("mlb_schedule_2014.csv",mode='a',header=False,index=False)     
